...
for i=1:k
    for j=1:10
        filename = strcat(src,'s',num2str(i),'\',num2str(j),'.pgm');
        I{i}{j} = double(imread(filename));
...
for ii=1:k
    ...
    for jj=1:10
       ...
       featMat=[];
      for p=1:countj
        featMat=[featMat hogfeat{p} ldbfeat{p}];
      end
      featMat=[featMat ii];
      FinalMat(count,:)=featMat;
      count=count+1;
    end
